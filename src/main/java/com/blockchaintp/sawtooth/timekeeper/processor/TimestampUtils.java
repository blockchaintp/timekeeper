...
public final class TimestampUtils {
  ...
  /**
   * Return the maximum timestamp given a previous timestamp and a list of timestamps.
   *
   * @param currentMax
  ...
  public static Timestamp median(final Collection<Timestamp> timestamps) {
    long medianValue;
    final List<Long> sortedTs = timestamps.stream().map(Timestamp::getSeconds).sorted().collect(Collectors.toList());
    if (sortedTs.isEmpty()) {
      ...
      final int half = (sortedTs.size() / 2);
  ...
  /**
   * Return the median of a list of timestamps or the last timestamp, whichever is greater.
   *
   * @param last
   *          the last timestamp
   * @param timestamps
